...
class FatigueDetect:
    ...
    def add_state(self, left_eye_state, right_eye_state, mouth_state):
        ...
        mouth_radio = np.sum(np.array(self.mouth_state_list[-self.mouth_jdg_count:]) == 1) / (
            self.mouth_jdg_count if len(self.mouth_state_list) > self.mouth_jdg_count else len(self.mouth_state_list))
        # 处理结果看是否疲劳
...
def image_add_tag(frame, refer_res, fatigue_status, frame_rate=0, debug=False):
    ...
    if debug:
        ...
        if show_pfld_info:
            for i in range(98):
                radius = 1
                draw.ellipse((refer_res["face_landmarks"][i * 2] - radius,
                              refer_res["face_landmarks"][i * 2 + 1] - radius,
                              refer_res["face_landmarks"][i * 2] + radius,
                              refer_res["face_landmarks"][i * 2 + 1] + radius), (0, 255, 127))
    ...
    draw.text((10, 50), "状态 : " + ("疲劳" if fatigue_status else "正常"),
              fill=(color_abnormal if fatigue_status else color_normal), font=font_20)
    if fatigue_status:
...
def gen_from_stream(sample_dir="", sample_type="refer", model_type="rknn", only_show=False, video_name=""):
    # 选择采样数据
    sample_handler = None
    if sample_dir != "":     # (处理样本数据的函数？我们用不上吧)
        sample_handler = SampleFaceData(sample_dir)

    # 加载推理模型
    refer_handler = ReferImage(session_type=model_type)  # ！去refer_image里改模型

    # 疲劳检测  （不需要了，保留处理摄像头数据的功能）
    if video_name != "":
        # 视频处理较慢，减半判断的帧数
    ...
    while cap.isOpened():
        ...
        # 推理 （用修改后的模型推理功能，模型修改要提供.refer的接口函数）
        ...
        fatigue_status = fatigue_detect.add_state(refer_ret["eye_left_status"], refer_ret["eye_right_status"],
                                                  refer_ret["mouth_status"])
        ...
        # 在图片上显示疲劳提示信息（眼睛的框、帧率等）（改image_add_tag函数）
        now_time_t = time.time()
        frame_rate = 1.0 / (now_time_t - last_frame_t)  #计算帧率--liuhuayuan
        last_frame_t = now_time_t
...
def video_feed(video_name):
    ...
    if ".13." not in IP_ADDRESS:
        refer_model = "onnx"
    return Response(gen_from_stream(video_name=video_name, model_type=refer_model),
                    mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/camera_feed')  # 主要，往前端返回结果
def camera_feed():
...
def sample_picture_data():
    ...
    return Response(gen_from_stream(sample_dir=sample_dir, sample_type="sample"),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
